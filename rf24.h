...
void setupradio(){
    // set CSN pin to output
    DDRB |= _BV(CSN_PIN);
    DIGIWRITE_L(PORTB, CSN_PIN);
};

void togglecsn(bool mode)
{
	uint8_t pinstatus = 0;
    pinstatus = !pinstatus;
    if(pinstatus)
    {
        DIGIWRITE_H(PORTB, CSN_PIN);
        _delay_us(100);
    }
    else
    {
...
{
    SPISettings(RF24_SPI_SPEED, MSBFIRST, SPI_MODE0);
    csn(LOW);
};
...
uint8_t read_rf24_register(uint8_t reg, uint8_t* buf, uint8_t len)
{
  uint8_t status;
  beginTransaction();
...
uint8_t read_rf24_register(uint8_t reg)
{
  uint8_t result;
  beginTransaction();
...
uint8_t write_rf24_register(uint8_t reg, const uint8_t* buf, uint8_t len)
{
  uint8_t status;
  beginTransaction();
...
uint8_t write_rf24_register(uint8_t reg, uint8_t value)
{
  uint8_t status;
  beginTransaction();
  status = SPItransfer( W_REGISTER | ( REGISTER_MASK & reg ) );
...
uint8_t write_rf24_payload(const void* buf, uint8_t data_len, const uint8_t writeType)
{
  uint8_t status;
